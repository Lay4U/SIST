...
public class Ex05_PreparedStatement {
	public static void main(String[] args) {
		...
		// values(seqAddress.nextval, '홍길동', 20, 'm', '서울시 강남구 역삼동');

		// Statement vs PreparedStatement
		// - 정적 쿼리(매개변수가 없는 쿼리): Statement or PreparedStatement 동일 > Statement 사용 권장 (간단하기에)
...
//		정적 or 동적 ? > 동적 쿼리 > PreparedStatement
	...
	private static void m2() {
		...
		try {
			// SQL > ? > 오라클 매개변수(오라클 문법) == String.foramt()과 비슷한 역할
			...
			// 매개변수를 대입하는 과정에서 유효하지 않은 문자를 알아서 예외 처리를 한다. > 안전성 향상
	...
	private static void m1() {
		...
		try {
			...
			String sql = String.format("insert into tblAddress values(seqAddress.nextVal, '%s', %d, '%s', '%s')", name,
					age, gender, address);
