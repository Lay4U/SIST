...
<body>
	<div class="container">
		...
		<c:if test="${not empty id}">
			<button type="button" class="btn btn-default"
			onclick="location.href='/jsp/auth/member.do';">회원정용 페이지</button>
		</c:if>
		<hr>
		<c:if test="${ empty id }">
		<button type="button" class="btn btn-default"
		...
		<c:if test="${ not empty id }">
		<button type="button" class="btn btn-default" onclick="location.href='/jsp/auth/admin.do';">관리자 전용 페이지 </button>
		</c:if>
	</div>	
	<script>
