...
// - 톰캣은 프로젝트 소스를 직접 실행치 않고 복사본 가지고 실행한다.
// - 업로드를 저장할 폴더가 톰캣 실행하는 작업 폴더를 지정해야한다.
...
// 기존의 request 객체의 getparameter()가 정상동작을 하지 않는다.(인코딩 변경으로 인해서..)
...
try {
	...
	MultipartRequest multi = new MultipartRequest(request, //기존의 request Wrapping 클래스);
	path, //업로드 폴더 지정
	size, //업로드 크기 지정
	"UTF-8", //인코딩 지정
...
<body>
	...
	<div class="container">
		...
		<!-- 브라우저는 링크 대상이 자신이 열 수 있는 파일이면 해석해서 보여주고,(텍스트, 웹페이지, 이미지 등) 열 수 없는 파일이면 다운로드를 한다. -->
		<div><a href="/jsp/files/<%=filename%>"><%= filename %></a></div>

		<!-- <a download> 속성이 파일을 무조건 다운로드되게 해준다. -->
		<div><a href="/jsp/files/<%=filename%>" download><%= filename %></a></div>
