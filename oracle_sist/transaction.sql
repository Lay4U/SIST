/*
 ...
 - 오라클에서 발생하느 1개 이상의 명령들을 하나의 논리 집합으로 묶어놓은 단위 . (실행된) 명령어들의 집합 > 통제(제어)
 - 트랜잭션에 포함되는 명령어 > insert, updete, delete > 테이블을 조작할 수 있는 명령어

 트랜잭션 명령어
 - DCL(TCL)
 - 하나의 트랜잭션으로 묶여있는 DML들을 감시하다가, 일부 DML에서 문제가 발생하며 이전에 성공한 모든 DML을 어떻게 처리할것인가?
 1. commit
 2. rollback
 3. savepoint
 ...
 4. ddl을 실행하는 경우(create, alter, drop)
 */
drop table tbltrans;
...
commit; -- 현재 트랜잭션의 모든 작업을 실제 데이터베이스(오라클)에 저장하라..
