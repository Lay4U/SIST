...
// - for in -> 객체가 가지는 프로퍼티를 탐색하는 전용 for문
...
hong.info = function() {
    console.log('저는 %s입니다. 나이는 %s세입니다.', hong.name, hong.age);
};
...
// 자바스크립트: 지역 변수(***), 전역 변수(***)
// - 자바스크립트의 지역 변수는 함수만 인식한다.(제어문은 인식하지 못함.)
...
// 변수 선언 키워드(var) 없이도 변수를 선언 가능하다. (이유가 있다.)
// -> 사용 금지!!!!(가독성 저하)
color = '노랑';
console.log(color);
...
function test() {
    ...
    height = 200; //지역변수(test())
}

// console.log(width);
// console.log(height); //옛날에는 됐다
...
// 생성자 함수는 단독 호출(X) -> 반드시 new 연산자와 함께 사용한다.
var user = new User();
console.log(user);
// user.hello();
...
var now = new Date(); // Date라는 이름의 생성자 함수
