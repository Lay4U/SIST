...
public class Ex25_String {
	...
	private static void m14(BufferedReader reader) {
		...
		//charAt() or substring()
		// -> 추출한 글자수에 따라서
		// 1글자 추출 -> charAt() or substring() -> 문자 코드이 필요
		// N글자 추출 -> substring()
		
		
		//자주 사용하는 예제
		String path = "T:\\java\\JavaTest\\src\\com\\test\\java\\Hello.java";
	...
	private static void m10(BufferedReader reader) {
		...
		String txt = "안녕홍길동하세요. 홍길동님. 반갑습홍길동니다. 홍길동님.sdfasfde홍길동lkjlk홍길동lkjlk홍길동lkj홍길동lkj홍길동lkj홍길동";
	...
	private static void m8(BufferedReader reader) throws IOException {
		...
		txt = "전날 전국적으로 많은 비가 내린 가운데 13~14일 반짝 추위가 찾아 올 전망이다. 일부 지역에서는 아침 기온이 오늘보다 10도 이상 떨어져 '한파특보'가 발효될 것으로 예상된다.\r\n"
				+ "\r\n"
				...
				+ "\r\n"
				+ "기상청 관계자는 \"내일은 중부내륙과 강원산지·경북북동산지·전북동부에서, 모레는 중부내륙·강원산지·남부내륙에서 기온이 영하로 떨어지는 곳이 많겠다\"며 \"서리가 내리고 얼음이 어는 곳이 있겠으니 과수나 작물 등 개화 후 수분이 일어나는 시기에 냉해를 입지 않도록 농작물 관리에 각별히 유의하기 바란다\"고 말했다.";
	...
	private static void m5(BufferedReader reader) throws Exception {
	...
	private static void m3(BufferedReader reader) {
		...
		//java.lang.StringIndexOutOfBoundsException: index 6, length 6
