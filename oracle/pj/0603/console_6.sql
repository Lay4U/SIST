...
select *
from TESTATTEND
where to_char(outtime, 'hh24') <= 17
  and to_char(outtime, 'mi') <= 55;
select to_char(outtime, 'hh24')
from testattend;
update testattend
set ATTENDANCE = '정상'
where attendSeq in
      (select attendSeq from TESTATTEND where to_char(outtime, 'hh24') <= 17 and to_char(outtime, 'mi') <= 55);
...
where CLASSSEQ = null;
alter table tblattend
    add FOREIGN KEY (classseq) REFERENCES TBLCLASS (CLASSSEQ);
commit;
...
select * from tblattend where ATTENDANCE not in ('지각', '정상');
